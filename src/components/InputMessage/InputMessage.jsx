...
const InputMessage = () => {
  ...
  const [audioRecordingCancelled, setAudioRecordingCancelled] = useState(false);
  ...
  const startAudioRecording = async () => {
    try {
      ...
      setAudioRecordingCancelled(false);
  ...
  const cancelAudioRecording = () => {
    if (mediaRecorder) {
      ...
      // Restablecer el estado relacionado con la grabación de audio
      setAudioRecordingText(""); // Limpiar el texto de grabación de audio
      setRecordingTimer(0); // Restablecer el temporizador de grabación
      setAudioRecordingCancelled(true);
    }
  };
  ...
  const handleSend = async () => {
    ...
    if (audio && !audioRecordingCancelled) {
    ...
    try {
      setIsSending(true); // Indicar que se está enviando el mensaje
      ...
      setIsSending(false); // Indicar que el envío se ha completado (incluso en caso de error)
    }
  };
    ...
    <div className='input'>
      ...
      {isAudioRecording && ( // Muestra el componente de grabación de audio cuando se está grabando
        <div className="audioRecordingContainer">
          ...
          <div className='send audioSend' onClick={handleSend} >
            {isSending ? (
              <MessageLoader />
            ) : (
              <Icon 
